						...
						<table>
							<thead>
								<tr>
									...
									<g:sortableColumn property="type" title="${message(code: 'badge.type.label', default: 'Type')}" />
								
									<th><g:message code="badge.user.label" default="User" /></th>
							...
							<g:each in="${badgeInstanceList}" status="i" var="badgeInstance">
								<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
									...
									<td><g:link action="show" id="${badgeInstance.id}">${fieldValue(bean: badgeInstance, field: "type")}</g:link></td>
								
									<td>${fieldValue(bean: badgeInstance, field: "user")}</td>
